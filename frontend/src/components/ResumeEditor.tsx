...
const requestCache = new Map<
  string,
  Promise<{ data: ResumeData; html?: string }>
>();
...
export function ResumeEditor({
  ...
  const loadData = async (params?: {
    ...
    try {
      ...
      // If forcing or if not in cache, fetch
      if (params?.force || !requestCache.has(internshipId)) {
        if (params?.force) requestCache.delete(internshipId);

        dataPromise = fetch(
          `http://localhost:3000/api/internships/${internshipId}/tailor-content`,
          {
            method: "POST",
        ...
        dataPromise = requestCache.get(internshipId)!;
      }

      const res = await dataPromise;
      setResumeData(res.data);

      // If forcing or no HTML source yet, generate/set source
      // Note: If previously saved, we use it UNLESS force is true?
      // Actually backend logic: if force is true, backend regenerates data.
      // So if force is true, we should definitely regenerate HTML from new data.
      if (res.html && !params?.force) {
  ...
  const saveResumeState = async (htmlToSave: string) => {
    ...
    await fetch(
      `http://localhost:3000/api/internships/${internshipId}/save-resume`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      let htmlToSave = showCodeEditor
        ? htmlSource
        : getCleanHtmlFromIframe() || htmlSource;
      ...
      const res = await fetch(
        "http://localhost:3000/api/generate-pdf-from-html",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <div className="bg-surface w-full max-w-[95vw] h-[95vh] rounded-2xl shadow-2xl flex flex-col border border-outline/10">
        ...
        <div className="flex items-center justify-between p-4 border-b border-outline/10 bg-surface rounded-t-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Re-tailor with Instructions"
            ...
            <button
              onClick={onClose}
              className="p-2 text-on-surface-variant hover:bg-surface-variant/50 rounded-full"
            >
              <X size={24} />
            </button>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
              ...
              <div className="p-6">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Provide any extra details, project links, or specific
                  instructions for the AI to include in this version of the
                  resume.
                </p>
                <textarea
                  value={additionalInfo}
                  onChange={(e) => setAdditionalInfo(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none min-h-[120px]"
                  placeholder="e.g. Include my project 'SmartRepo' (github.com/...), emphasize my React experience..."
                />
                <div className="mt-6 flex justify-end gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      loadData({ additionalInfo, force: true });
                    }}
                    disabled={loading}
        ...
        <div className="flex-1 bg-surface-variant/20 overflow-hidden relative flex flex-row">
          ...
          >
            ...
            {editingLink && (
              <div
                className="absolute z-30 p-4 bg-white rounded-lg shadow-xl border border-gray-200 w-80 animate-in fade-in zoom-in duration-200"
                style={{
                  top: "50%",
                  left: "50%",
