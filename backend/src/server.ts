...
const CONFIG = {
  internshipListUrl:
    "https://internshala.com/internships/work-from-home-backend-development,front-end-development,full-stack-development,javascript-development,node-js-development,software-development,web-development-internships/",
...
app.post("/api/internships/:id/retry-ai", async (req, res) => {
  ...
  try {
    ...
    if (!internship.matchAnalysis) {
      ...
      aiExtractAndMatchResult =
        (await aiService.extractAndMatch(
    ...
    if (!existingCompanyData && internship.companyDetailPageUrl) {
      ...
      const companyDetails = await plugin.fetchCompanyDetails(
        internship.companyDetailPageUrl
      );
    ...
    if (!existingCompanyData?.analysis) {
      ...
      aiCompanyAnalysisResult =
        (await aiService.analyzeCompany(
          internship.company,
...
app.post("/api/companies", async (req, res) => {
  ...
  try {
    ...
    if (!existing?.analysis) {
      const analysis = await aiService.analyzeCompany(
        name,
        details.location,
        details.about
      );
...
app.post("/api/extension/analyze", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to analyze company" });
  }
});

app.get("/api/internships/:id", (req, res) => {
  try {
    const internship = storage
      .getInternships()
  ...
  try {
    ...
    if (presetName) {
      ...
      if (urls) {
        ...
        sendEvent({
          type: "status",
          message: `Using preset: ${presetName}...`,
        });
    ...
    const newListings = allListings.filter(
      (listing) =>
        !storage.isProcessed(listing.id) &&
        !storage.isBlacklisted(listing.company)
    );

    console.log(`Found ${newListings.length} new internships.`);
    sendEvent({
      type: "status",
      message: `Found ${newListings.length} new internships to process.`,
    });
    ...
    for (const listing of newListings) {
      ...
      sendEvent({
        type: "status",
        message: `Extracting details & matching resume...`,
      });

      if (!rawDetails) {
        sendEvent({
          type: "status",
          message: `Failed to fetch details for ${listing.company}`,
        });
        continue;
      }

      const aiResult = await aiService.extractAndMatch(
        rawDetails.meta,
      ...
      if (company) {
        ...
        sendEvent({
          type: "status",
          message: `Using cached analysis for ${listing.company}...`,
        });
      } else {
        ...
        sendEvent({
          type: "status",
          message: `Verifying company ${listing.company}...`,
        });
        ...
        companyDetails =
          (await pluginManager.fetchCompanyDetails(
            listing,
            rawDetails.companyDetailPageUrl
          )) ?? undefined;

        if (!companyDetails) {
          sendEvent({
            type: "status",
            message: `Failed to fetch company details for ${listing.company}`,
          });
        }

        companyAnalysis = await aiService.analyzeCompany(
          listing.company,
...
app.post("/api/internships/:id/tailor-content", async (req, res) => {
  const { id } = req.params;
  const { additionalInfo, force } = req.body; // Extract params
  ...
  try {
    ...
    // 1. Check for saved resume (Skip if force is true)
    if (!force && (internship.savedResumeData || internship.savedResumeHtml)) {
      console.log(`Returning saved resume for ${internship.company}`);
      return res.json({
        success: true,
        data: internship.savedResumeData,
        html: internship.savedResumeHtml,
      });
    }

    console.log(
      `Generating tailored resume content for ${internship.company}...`
    );
    ...
    if (!fs.existsSync(resumePath)) {
      return res
        .status(400)
        .json({ error: "No resume found (uploads/resume.pdf)" });
    }
    ...
    // 5. Generate Structured Resume Data (Pass additionalInfo)
    const resumeData = await aiService.tailorResume(
      description,
      resumeText,
      userProfile,
      additionalInfo
    ...
    if (!resumeData) {
      return res
        .status(500)
        .json({ error: "AI failed to generate resume data" });
    }
...
app.post("/api/generate-pdf-from-html", async (req, res) => {
  ...
  try {
    ...
    const safeCompany = (company || "Unknown")
      .replace(/[^a-z0-9]/gi, "_")
      .substring(0, 30);
    const safeRole = (title || "Internship")
      .replace(/[^a-z0-9]/gi, "_")
      .substring(0, 30);
